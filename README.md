...
- 从菜单新建独立串口标签，每个标签保留自己的配置、连接和终端内容
...
- `Enter`：发送当前内容
- `文件 > 新建串口标签`：创建一个独立串口会话
- `会话` 菜单：连接、刷新端口、暂停接收和清空终端
- `显示` 菜单：切换 ASCII / HEX、时间戳和自动滚动
- 标签内的配置菜单：选择端口、波特率、数据位、停止位、校验位和流控制
