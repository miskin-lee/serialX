...
- 每一行左侧先是时间戳、再是行号：行号像编辑器一样从 1 起编，回滚满了以后旧行丢弃但余下的行号不变（屏幕上第一行的号码会越过 1），清屏（菜单或设备发来的 `clear`）后重新从 1 起编；时间戳记录该行第一个字节到达的本地时间（精确到毫秒），终端自动折行的续行不重复显示；标题栏过滤框默认高亮匹配的行并统计屏幕上的匹配数，打开遮蔽开关后只显示命中的行
...
- VS Code 式标题栏（菜单栏）：中央是命令中心，左右箭头切换标签，过滤框按纯文本或正则表达式筛选终端输出，右侧是当前标签的连接 / 断开按钮，
  ...
  过滤有两种模式：默认高亮命中的行、其余行原样保留，过滤框右端的“闭眼”开关打开遮蔽模式，只显示命中的行——从整段回滚里取，带各自的行号和时间戳，方便看出中间隔了多少行——计数也改为整段日志里的“命中行数 / 总行数”；遮蔽视图独立滚动，查找、选择和复制都只针对显示出来的行，没有任何行命中时终端里会说明；
