...
- 每个会话都有一种标签色：新建或编辑会话时在 24 种颜色中选一种（两排：一排明亮色相，一排更深更饱和的颜色），新会话默认从当前没有标签页在用的颜色里随机取一种；标签页的底板和右侧已保存会话的图标都会带上这个颜色，连接状态仍由标签上的圆点表示
...
- 右侧 `Sessions`：保存或恢复会话配置，双击打开，铅笔修改、垃圾桶删除（删除前弹出确认，`Enter` 确认，`Esc` 保留）；右键分区标题或列表新建分组；已在标签中打开的端口会显示绿点
