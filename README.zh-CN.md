...
- 每一行左侧有行号和时间戳：行号像编辑器一样从 1 起编，回滚满了以后旧行丢弃但余下的行号不变（屏幕上第一行的号码会越过 1），清屏（菜单或设备发来的 `clear`）后重新从 1 起编，`⌘⇧N` 或 `View` 菜单可以关掉；时间戳记录该行第一个字节到达的本地时间（精确到毫秒），终端自动折行的续行不重复显示；标题栏过滤框默认高亮匹配的行并统计屏幕上的匹配数，打开遮蔽开关后只显示命中的行
