...
/// An opaque Objective-C class, only ever seen behind a pointer.
///
/// Upstream declares this as an empty `enum`, and rustc's
/// `uninhabited_static` lint (rust-lang/rust#74840) rejects an `extern`
/// static of an uninhabited type, since one could never be initialised. A
/// zero-sized struct is inhabited, has the same layout at the ABI, and is
/// what `objc` and `block2` use for the same purpose.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
