...
- 终端下方的输入栏集成 UTF-8 / HEX 分段开关（HEX 模式把输入解析为十六进制字节发送），支持 Enter 快速发送
...
- 终端上方只有一条标签栏：左侧是会话标签（连接状态点、端口名、关闭），右端只有连接 / 断开一个按钮；暂停接收、清屏、时间戳、自动滚动和刷新端口都在 `Session` / `View` 菜单里，各有快捷键
